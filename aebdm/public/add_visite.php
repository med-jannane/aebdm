...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $stmt = $pdo->prepare('INSERT INTO visites_preventives (code_client, code_contrat, details, statut, user_id) VALUES (?, ?, ?, ?, ?)');
        $stmt->execute([$code_client, $code_contrat, $details, $statut, $user_id]);
        $visite_id = $pdo->lastInsertId();
            ...
            for ($i = 0; $i < count($_FILES['fichiers']['name']); $i++) {
                if ($_FILES['fichiers']['error'][$i] === 0) {
                    ...
                    if (move_uploaded_file($_FILES['fichiers']['tmp_name'][$i], $file_path)) {
                        ...
                        $stmt->execute([$visite_id, $file_name, $file_extension]);
                    }
                }
            }
        }
        header('Location: visites.php?message=Visite préventive ajoutée avec succès !');
        exit;
...
<head>
    ...
    <title>Ajouter une visite préventive - AEBDM</title>
...
<body>
    ...
    <div class="container">
        <div class="page-header">
            <h2>Ajouter une visite préventive</h2>
            <a href="visites.php" class="btn-edit"><i class="fas fa-arrow-left"></i> Retour</a>
        </div>
        <?php if ($message): ?>
        ...
        <form method="post" enctype="multipart/form-data" class="modern-form">
            <div class="form-grid">
                ...
                <div class="form-group full-width">
                    ...
                    <textarea name="details" rows="4" required placeholder="Détails de la visite"></textarea>
                </div>
                <div class="form-group">
            ...
            <div class="form-actions">
                <button type="submit" class="btn-add"><i class="fas fa-plus"></i> Ajouter la visite</button>
                <a href="visites.php" class="btn-edit"><i class="fas fa-times"></i> Annuler</a>
            </div>
        </form>
    </div>
